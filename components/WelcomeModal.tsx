...
import { ShieldAlert, Globe, Smartphone, Monitor, Watch, CheckCircle2, Lock, Facebook, Mail, MonitorSmartphone, Zap } from 'lucide-react';
      ...
      <div className="relative w-full max-w-4xl bg-[#020617] border border-brand-500/30 rounded-[2.5rem] md:rounded-[3.5rem] shadow-[0_0_150px_rgba(234,179,8,0.15)] overflow-hidden animate-bounce-in flex flex-col max-h-[98vh] md:max-h-[90vh] ring-1 ring-white/10">
        ...
        <div className="p-5 md:p-14 overflow-y-auto custom-scrollbar relative z-10">
          ...
          <div className="text-center mb-8 md:mb-12 relative">
            ...
            <h2 className="text-3xl md:text-6xl font-display font-black text-white leading-none tracking-tighter uppercase mb-4 italic">
          ...
          {/* GRID "NUNCA" - 3 COLUNAS */}
          <div className="grid grid-cols-3 gap-2 md:gap-8 mb-8 md:mb-14">
            {[
              { label: "CONTA FACEBOOK", icon: <Facebook className="w-6 h-6 md:w-14 md:h-14" />, color: "from-blue-600/25" },
              { label: "CONTA GMAIL", icon: <Mail className="w-6 h-6 md:w-14 md:h-14" />, color: "from-red-600/25" },
              { label: "SEU DISPOSITIVO", icon: <MonitorSmartphone className="w-6 h-6 md:w-14 md:h-14" />, color: "from-emerald-600/25" }
            ].map((item, i) => (
              <div key={i} className={`relative group flex flex-col items-center text-center p-4 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] bg-gradient-to-b ${item.color} to-black/80 border border-white/5 hover:border-brand-500/50 transition-all duration-700 shadow-2xl transform hover:-translate-y-1`}>
                <div className="text-white mb-3 md:mb-6 opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-all">
                    {item.icon}
                </div>
                <div className="space-y-1 md:space-y-2">
                    <span className="text-red-500 font-black text-[7px] md:text-[11px] uppercase tracking-[0.1em] md:tracking-[0.2em] block leading-none">NUNCA</span>
          ...
          <div className="bg-gradient-to-br from-brand-500/5 to-transparent border border-brand-500/10 p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] mb-8 md:mb-12 relative overflow-hidden group shadow-inner">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-5 md:gap-8 relative z-10">
                ...
                <div className="text-center md:text-left max-w-xl">
                    <h4 className="text-brand-400 font-black text-xs md:text-sm uppercase tracking-[0.2em] md:tracking-[0.3em] mb-2">SISTEMA 100% CLOUD-BASED</h4>
          ...
          {/* COMPATIBILIDADE */}
          <div className="flex flex-col sm:flex-row justify-between items-center gap-6 bg-black/40 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-white/5">
             <div className="flex items-center gap-4">
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-slate-800 flex items-center justify-center">
                    <Zap className="text-brand-500 w-5 h-5 md:w-6 md:h-6 fill-brand-500" />
                </div>
                <div>
                    <h4 className="text-white font-bold uppercase tracking-widest text-[11px] md:text-sm">DISPONÍVEL EM:</h4>
                    <p className="text-slate-500 text-[9px] md:text-xs font-mono uppercase tracking-tighter">MULTI-PLATAFORMA ESTÁVEL</p>
                </div>
             </div>
             
             <div className="flex gap-6 md:gap-10 opacity-30 group-hover:opacity-50 transition-opacity">
                <Smartphone className="w-5 h-5 md:w-7 md:h-7 text-white" />
                <Monitor className="w-5 h-5 md:w-7 md:h-7 text-white" />
                <Watch className="w-5 h-5 md:w-7 md:h-7 text-white" />
                <Zap className="w-5 h-5 md:w-7 md:h-7 text-brand-500" />
             </div>
          </div>
        </div>
        ...
        <div className="p-6 md:p-12 bg-black/80 border-t border-white/5 mt-auto flex flex-col items-center shrink-0">
          <button 
            onClick={onClose}
            className="w-full max-w-md group relative py-4 md:py-5 bg-brand-600 hover:bg-brand-500 text-black font-display font-black text-lg md:text-xl uppercase tracking-[0.2em] md:tracking-[0.3em] rounded-[1.25rem] md:rounded-[1.5rem] shadow-neon transition-all transform active:scale-95 flex items-center justify-center gap-3 md:gap-4 border border-white/10"
          >
            <CheckCircle2 className="w-6 h-6 md:w-8 md:h-8" />
            INICIAR AGORA
          </button>
          
          <div className="flex items-center gap-2 md:gap-3 mt-4 md:mt-6 opacity-20">
             <Lock className="w-3 h-3 text-emerald-500" />
             <p className="text-[8px] md:text-[10px] text-slate-500 uppercase font-black tracking-[0.2em] md:tracking-[0.4em] whitespace-nowrap">
